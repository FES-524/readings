...
Having wide confidence intervals is a common problem when an analysis ends up with inconclusive results.  The size of the confidence interval is affected by the standard errors.  Standard errors are affected by the sample size.
...
Given the same standard deviation (SD), a larger sample will reduce the standard error and so the size of confidence intervals. There is one **very important** nuance to this, though. Sample size is not the only thing that affects the standard error. You can collect infinitely many samples from a study in which your protocol or variable of interest is confounded with another variable, such as the blocking factor discussed in reading 3.1. If both are in the model, the width of the CI will remain infinitely large! Thus, increasing your sample size is a good thing *if* you have a solid experimental / sampling design.
...
The affect of unexplained variation on the results is why we spent time talking about sources of variation.  If we can control the variation via study design or explain it through covariates, we can get a more precise answer without having to sample more units.
...
The representativeness of the sample to the population of inference could be a limitation you want to discuss.  For example, you could question how representative a very small sample really is of the population. The smaller the sample, the greater an effect "odd" observations, while true to the population of interest, will have on your results.  
